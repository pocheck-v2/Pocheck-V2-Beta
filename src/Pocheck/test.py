...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        classifier_filename = '../parameter/clf/c_clf.pkl'
            ...
            if (c % timeF == 0):
                ...
                if nrof_faces > 0:
                    ...
                    for i in range(nrof_faces):
                        ...
                        cropped[i] = facenet.flip(cropped[i], False)    # 전치 해준다. (왜하지 )
                        scaled.append(misc.imresize(cropped[i], (image_size, image_size), interp='bilinear'))   #182 size
                        scaled[i] = cv2.resize(scaled[i], (input_image_size, input_image_size),
                                               interpolation=cv2.INTER_CUBIC)   # 160 size
                        ...
                        max_v = -99999999999
                        predictions = []
                        for i in range(emb_result.shape[0]):
                            value = Verify(A, G, emb_array, emb_result[i])
                            predictions.append(value)
                            if max_v < value:
                                max_v = value
                                max_i = i

                        # print("emb_array : ", emb_array, emb_array.shape)
                        # predictions = model.predict_proba(emb_array)

                        # best_class_indices = np.argmax(predictions, axis=1)

                        best_class_indices = max_i
                        ...
                        # if best_class_indices > 20:
                        #     continue
                        # print('result: ', HumanNames[best_class_indices], str(np.round(best_class_probabilities, 2)))
                        print(max_i, paths[max_i])
                        '''
